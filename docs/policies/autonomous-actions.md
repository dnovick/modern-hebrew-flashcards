...
| 5 | **Git push (feature branch)** | 2 | `git push` to the current feature branch | Pushing to `main` is blocked by branch protection and is not a promotable action |
...
| 8 | **GitHub — PR creation** | 2 | `gh pr create` | Every change goes through a PR, so opening one is the normal path rather than an exception. Report the link |
| 9 | **GitHub — PR merge** | 1 | `gh pr merge --squash` | Owner says so in chat. Never with CI red, never unasked |
...
- Pushing to `main` (enforced by branch protection since 2026-09-14)
- Merging a PR
